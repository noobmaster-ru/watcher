...
const LAUNCH_ARGS = [
  // --no-sandbox неизбежен: без него Chrome в контейнере не стартует, а
  // сторонний seccomp-профиль ломал запуск самого контейнера (старый профиль
  // не знал statx, нужный runc). Жёлтую плашку про флаг прячем: она рисуется
  // в интерфейсе браузера, а не на странице, и антибот её не видит.
  "--no-sandbox",
