...
export function retryDelayMin(errorCount: number, intervalMin: number): number {
  // Повторяем раньше обычного срока, но не раньше, чем лимит успевает отпустить:
  // замер показал, что search.wb.ru приходит в себя примерно за полчаса тишины,
  // и повтор через 10 минут гарантировал бы ещё один отказ. Пауза растёт с
  // каждой неудачей, но никогда не превышает обычный интервал проверки.
  return Math.min(intervalMin, 30 * 2 ** Math.max(errorCount - 1, 0));
}
