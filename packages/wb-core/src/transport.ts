...
// Замерено на боевом сервере: после серии запросов search.wb.ru отвечает 429
// и через 10, и через 20 минут тишины, а на 30-й минуте отдаёт 200. Поэтому
// пауза после отказа начинается с получаса — меньшая просто гарантирует ещё
// один 429 и продлевает штраф.
const SLOW_BASE_BACKOFF_MS = 30 * 60_000;
const SLOW_MAX_BACKOFF_MS = 2 * 60 * 60_000;
